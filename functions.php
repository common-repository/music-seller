...
function music_seller_meta_box_callback( $post ) {
	...
	$order = get_post_meta(get_the_ID(),'music_seller_order',true);//
	...
	if (count($uploads) > 0 && is_array($uploads)) {
		...
		$wavs = get_post_meta(get_the_ID(),'music_seller_wavs',true);
		foreach ($uploads as $id => $row) {
			$filename = basename($row['file']);
			?>
			<div class="music_seller_row">
			<div class="music_seller_left">
				<i class="music_seller_icon"></i>
				Order:<br />
				<input type="number" class="music_seller_order" name="music_seller_order[<?php echo $id; ?>]" value="<?php echo @$order[$id]; ?>" size=3 />
			</div>

			<span><b>File for sale: </b><?php echo $filename; ?><br />
				<?php
				// if (MUSIC_SELLER_VERSION == 'Full') {
					...
					if (isset($previews[$id]['url'])) {
						...
						echo '<span class="music_seller_no_preview">Upload a preview that will be played before buying:</span>';
					}
			?>
				<input type="file" name="music_seller_preview[<?php echo $id; ?>]" /></span>
<?php
				if (MUSIC_SELLER_VERSION == 'Full') {
					if (isset($wavs[$id]['url'])) {
						echo '<br /><b>FLAC/WAV file: </b> <a class="music_seller_preview_uploaded" href="' . $wavs[$id]['url'] . '">' . basename($wavs[$id]['file']) . '</a><br />Reupload WAV/FLAC: ';
					} else {
						echo '<br /><span class="music_seller_no_wav">Upload a WAV/FLAC:</span>';
					}
					?><input type="file" name="music_seller_wavs[<?php echo $id; ?>]" /></span><?php
				} else { echo "<h4>Upgrade to the full version in order to be able to upload more than 3 files and have the ability to attach WAV / FLAC versions.<br /><a target=\"_blank\" href=\"https://www.shopfiles.com/index.php/products/music-seller-plugin-for-wordpress\">Click to UPGRADE to PRO</a></h4>"; }
?>
				<div class="music_seller_price">Title: <input size="20" name="music_seller_title[<?php echo $id; ?>]" placeholder="" value="<?php echo ($titles[$id] != '' ? $titles[$id] : basename($uploads[$id]['file'])); ?>" /> Price: <input size="6" name="music_seller_price[<?php echo $id; ?>]" placeholder="0.00" value="<?php echo (@$prices[$id] > 0 ? $prices[$id] : '0.00'); ?>" /> <label><input type="checkbox" class="music_seller_delete" title="Delete" name="music_seller_delete[]" value="<?php echo $id; ?>" /> <span class="music_seller_delete_icon" title="Delete this file"></span></label>
				</div>
			</div>
			<?php
		}
...
function music_seller_save_custom_meta_data($id) {
	...
	add_post_meta($id,'music_seller_order',$_REQUEST['music_seller_order']);
	update_post_meta($id,'music_seller_order',$_REQUEST['music_seller_order']);
	//music_seller_order
		...
		foreach ($_FILES['music_seller_file']['name'] as $fkey => $file) {
			if(!empty($file)) {
				...
				if(in_array($uploaded_type, $supported_types)) {
					...
					wp_die("Uploaded file is not in audio format." . print_r($_FILES));
				} // end if/else
	...
	$uploads = array();
	$upload_audio_types = array('music_seller_preview', 'music_seller_wavs');
	foreach ($upload_audio_types as $upload_audio_type) {
		if (!$_FILES[$upload_audio_type]['name']) {
			return true;
		}

		foreach ($_FILES[$upload_audio_type]['name'] as $fkey => $file) {
			if(!empty($file)) {
				...
				// Get the file type of the upload
				$arr_file_type = wp_check_filetype(basename($_FILES[$upload_audio_type]['name'][$fkey]));
				$uploaded_type = $arr_file_type['type'];
				...
				if(1) {
					...
					// Use the WordPress API to upload the file
					if ($_FILES[$upload_audio_type]['error'][$fkey] == 1) {
					...
					$upload = wp_upload_bits($_FILES[$upload_audio_type]['name'][$fkey], null, file_get_contents($_FILES[$upload_audio_type]['tmp_name'][$fkey]));
					$uploads[$fkey] = $upload;
					if(isset($upload['error']) && $upload['error'] != 0) {
						wp_die('There was an error uploading your file. The error is: ' . $upload['error']);
					} else {
						$uploadsMeta = get_post_meta($id,$upload_audio_type,true);
						if (is_array($uploadsMeta)) {
						...
						//wp_die(print_r($uploads,true) . '<br /><br />' . print_r(get_post_meta($id,'music_seller_file',true),true) . print_r($merged,true));
						add_post_meta($id, $upload_audio_type, $merged);
						update_post_meta($id, $upload_audio_type, $merged);
...
function music_seller( $atts ){
	...
	$music_seller_file = get_post_meta(get_the_ID(),'music_seller_file',true);
	$music_seller_order = get_post_meta(get_the_ID(),'music_seller_order',true);

		//wp_die(print_r($music_seller_order,true));
	$orders_so_far = 0;

	foreach ($music_seller_file as $key => $value) {
		...
		$order = $music_seller_order[$key];
		if ($order < 1 || $order == 0 || $order == '') {
			$orders_so_far++;
			$order = max($music_seller_order) + $orders_so_far + 1;
		}
		if ($mp3FilesAdded > 2 && MUSIC_SELLER_VERSION != 'Full') continue;
		$mp3files[$order] = "{
				title:\"" . $title . "\",
	...
	ksort($mp3files);
	//wp_die(print_r($mp3files,true));
	$mp3files = implode("\n", $mp3files);

	$key = md5(microtime(true));

	$music_seller_warning = '';
	if (MUSIC_SELLER_VERSION == 'Full') {
		...
		if (count($music_seller_file)  > 3) {
			$music_seller_warning = "<h4>The FREE version of Music Seller allows you to sell up to three files, if you want to remove this warning, please upgrade by clicking <a target=\"_blank\" href=\"https://www.shopfiles.com/index.php/products/music-seller-plugin-for-wordpress\">here</a>.</h4>";
		}
...
function music_seller_download_link($order) {
	...
	if (!$order['order_id']) {
		$order['order_id'] = $order_orig['postid'];
	}

	$files_meta = (array)get_post_meta($order['postid'],'music_seller_file',true);
	$wav_meta = (array)get_post_meta($order['postid'],'music_seller_wavs',true);
	for ($i=0; $i < $order['num_cart_items'];$i++) {

		$link_post = get_permalink($order['postid']);
		$k = $i+1;
		...
		$link_mp3 = add_query_arg(array('music_seller_download' => $order['item_number' . $k], 'txn_id' => $order['txn_id'], 'order_id' => $order['order_id']),$link_post);
		$link = '<a href="' . $link_mp3 . '">Download ' . $order['item_name' . $k] . '</a> (' . number_format(filesize($files_meta[$order['item_number' . $k]]['file']) / (1024*1024),2) . ' MB)';
		//print_r($order);
		if ($wav_meta[$order['item_number' . $k]]['file'] != '') {
			$link_wav = add_query_arg(array('music_seller_download' => $order['item_number' . $k], 'txn_id' => $order['txn_id'], 'order_id' => $order['order_id'], 'format' => 'wav'),$link_post);
			$link .= '<Br /><a href="' . $link_wav . '">' . @basename($wav_meta[$order['item_number' . $k]]['file']) . '</a> (' . number_format(filesize($wav_meta[$order['item_number' . $k]]['file']) / (1024*1024),2) . ' MB)';
		}
		$links[] = $link;
		//http://wp2.shopfiles.com/dadada/?music_seller_download=d67321a54f2e867da6263b558178b150&txn_id=4N736927V8537442R&order_id=912
	}
	return implode("<br />\n", $links);
}
...
function music_seller_process_download() {
	...
	if ($_REQUEST['music_seller_download'] != '') {
		...
		if ($_REQUEST['format'] == 'wav') {
			$files = get_post_meta($order['postid'][0],'music_seller_wavs',true);
		}
...
//define('MUSIC_SELLER_VERSION',hex2bin('46756c6e')); //was c
define('MUSIC_SELLER_VERSION', 'Full'); //was c
if (MUSIC_SELLER_VERSION == 'Full') {
